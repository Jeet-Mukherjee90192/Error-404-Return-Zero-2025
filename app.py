...
def index():
    ...
    sample_symbols = ["AAPL", "MSFT", "GOOGL", "AMZN", "TSLA", "META", "NVDA", "NFLX", "JPM", "V", "MA", "DIS", "PYPL", "INTC", "ORCL"]
    top_stocks = []

    for sym in sample_symbols:
        rand_symb = choice(sample_symbols)
        sample_symbols.remove(rand_symb)
        data = lookup(rand_symb)
        if data:
            top_stocks.append({"symbol": data["symbol"], "price": data["price"]})
    ...
    return render_template(
        ...
        top_stocks=top_stocks    # 👈 Add this
    )
